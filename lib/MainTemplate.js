...
class MainTemplate {
	...
	constructor(outputOptions, compilation) {
		...
		this.hooks = Object.freeze({
			renderManifest: {
				tap: util.deprecate((options, fn) => {
					compilation.hooks.renderManifest.tap(options, (entries, options) => {
							console.log('renderManifest 钩子注册，位置是MainTemplate')
			...
			beforeStartup: {
				tap: () => {
					throw new Error("MainTemplate.hooks.beforeStartup has been removed (use RuntimeGlobals.startup instead)");
				}
			},
			startup: {
				tap: () => {
			...
			afterStartup: {
				tap: () => {
					throw new Error("MainTemplate.hooks.afterStartup has been removed (use RuntimeGlobals.startup instead)");
				}
			},
			render: {
				tap: util.deprecate((options, fn) => {
