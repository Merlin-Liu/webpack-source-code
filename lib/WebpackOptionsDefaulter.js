...
class WebpackOptionsDefaulter extends OptionsDefaulter {
	constructor() {
		...
		this.set("cache", "call", (value, options) => {
			...
			if (value.type === "filesystem") {
				if (value.name === undefined) {
					value.name = (options.name || "default") + "-" + (options.mode || "production");
				}
				...
				if (value.cacheDirectory === undefined) {
					...
					if (!dir) {
						...
						value.cacheDirectory = path.resolve(dir, "node_modules/.cache/webpack");
					}
				}
		...
		this.set("output.module", "make", options => options.experiments.outputModule);
		...
		this.set("optimization.runtimeChunk", "call", value => {
			...
			if (value === true || value === "multiple") {
				return { name: entrypoint => `runtime~${entrypoint.name}` };
			}
			return value;
		});
		...
		this.set(
			...
			"make",
			options =>!!(options.recordsInputPath || options.recordsOutputPath || options.recordsPath)
		);
