	...
	process(options, compiler) {
		...
		if (typeof options.target === "string") {
			switch (options.target) {
				...
				case "web": {
					...
					new FetchCompileWasmPlugin({ mangleImports: options.optimization.mangleWasmImports }).apply(compiler);
		...
		if (!options.experiments.outputModule) {
			...
			if (options.output.libraryTarget === "module") {
				throw new Error("'output.libraryTarget: \"module\"' is only allowed when 'experiments.outputModule' is enabled");
			}
		}
		...
		if (options.experiments.importAwait || options.experiments.importAsync || options.experiments.topLevelAwait) {
			const InferAsyncModulesPlugin = require("./async-modules/InferAsyncModulesPlugin");
			new InferAsyncModulesPlugin({
				errorOnImport: options.experiments.importAsync ? false : (options.experiments.importAwait ? "await" : true)
			}).apply(compiler);
		}
