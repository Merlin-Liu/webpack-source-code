...
class Compilation {
	...
	_buildModule(module, callback) {
		const currentProfile = this.profile ? this.moduleGraph.getProfile(module) : undefined;
		currentProfile !== undefined && currentProfile.markBuildingStart();

		module.needBuild({ fileSystemInfo: this.fileSystemInfo }, (err, needBuild) => {
			...
			module.build(this.options, this, this.resolverFactory.get("normal", module.resolveOptions), this.inputFileSystem, err => {
				...
				this.cache.store(`${this.compilerPath}/module/${module.identifier()}`, null, module, err => {
					if (currentProfile !== undefined) {
	...
	_processModuleDependencies(module, callback) {
		...
		consola.warn(`下面开始解析并收集Module \`${module.rawRequest}\`所依赖的项目`)

		const forEachFn = (item, callback) => {
			this.handleModuleCreation(item, err => {
				consola.success(`\`${item.originModule.rawRequest}\`下的依赖项目\`${item.dependencies[0].userRequest}\`收集完成`)
		...
		asyncLib.forEach(sortedDependencies, forEachFn, errorFn);
	}
	...
	handleModuleCreation({ factory, dependencies, originModule, context }, callback) {
		...
		consola.info(`1⃣️ 2⃣️  进入\`handleModuleCreation\`方法，要创建的Module为\`${dependencies[0].userRequest}\`，执行\`factorizeModule\`方法`)
		// factorize =》 因式分解
		this.factorizeModule({ currentProfile, factory, dependencies, originModule, context },
			(err, newModule) => {
				...
				if (!newModule) return callback();

				if (currentProfile !== undefined) {
					moduleGraph.setProfile(newModule, currentProfile);
				}

				consola.info('1⃣️ 4⃣️  `Module`创建完成，将其加入`Compilation`的Module队列中')
				this.addModule(newModule, (err, module) => {
					if (err) {
						if (!err.module) err.module = module;
						this.errors.push(err);
					...
					consola.info(`1⃣️ 5⃣️  新创建的Module \`${module.rawRequest}\` 添加完成，执行\`Compilation\`的\`buildModule\`构建`)
	 ...
	 * @returns {void}
	 */
	_factorizeModule({ currentProfile, factory, dependencies, originModule, context }, callback) {
		if (currentProfile !== undefined) {
	 ...
	 * 前两个参数：
	 * context："/Users/liuguangfu/Desktop/study-code/webpack-source-code/debug"
	 * dependency：EntryDependency实例 =》继承自ModuleDependency类 =》继承自Dependency抽象类
	 * 			  EntryDependency {weak: false, optional: false, loc: {name: 'customFilename'}, …}
	 */
	addModuleChain(context, dependency, callback) {
		...
		consola.info('1️⃣ 1️⃣  进入addModuleChain方法，执行handleModuleCreation方法创建Module')
		this.handleModuleCreation(
			...
			err => {
				consola.warn(`Module \`${dependency.userRequest}\`所依赖的项目解析并收集完成`)
	 ...
	 * 作用是将模块的入口信息传递给模块链中，即addModuleChain
	 * 前三个参数
	 * context："/Users/liuguangfu/Desktop/study-code/webpack-source-code/debug"
	 * dep：EntryDependency实例 =》继承自ModuleDependency类 =》继承自Dependency抽象类
	 * name：customOutputFile，也就是entry的name
	 */
	addEntry(context, entry, name, callback) {
		this.hooks.addEntry.call(entry, name);

		// entryDependencies是一个Map
		// Map中的每一项都代表了一个入口，打包输出就会有多个文件
		...
		consola.info('1️⃣ 0️⃣  进入Compilation的addEntry方法中，执行Compilation的addModuleChain方法')
	...
	_rebuildModule(module, callback) {
		...
		this.buildModule(module, err => {
			...
			this.processModuleDependencies(module, err => {
				if (err) return callback(err);
				this.removeReasonsOfDependencyBlock(module, { dependencies: oldDependencies, blocks: oldBlocks });
			...
			for (const module of modules) {
				...
				if (warnings !== undefined) {
					for (const warning of warnings) {
						if (!warning.module) warning.module = module;
	...
	seal(callback) {
		...
		buildChunkGraph(this, (this.chunkGroups.slice()) /** @type {Entrypoint[]} */);
		...
		// webpack中未对optimizeTree钩子注册
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			if (err) {
				return callback(makeWebpackError(err, "Compilation.hooks.optimizeTree"));
			}
			...
			const cont = () => {
				...
				this.hooks.additionalAssets.callAsync(err => {
					if (err) {
						return callback(makeWebpackError(err, "Compilation.hooks.additionalAssets"));
					}
					this.hooks.optimizeChunkAssets.callAsync(this.chunks, err => {
						if (err) {
							return callback(makeWebpackError(err, "Compilation.hooks.optimizeChunkAssets"));
						}
						this.hooks.afterOptimizeChunkAssets.call(this.chunks);
						this.hooks.optimizeAssets.callAsync(this.assets, err => {
							if (err) {
								return callback(makeWebpackError(err, "Compilation.hooks.optimizeAssets"));
							}
							...
							this.hooks.finishAssets.callAsync(this.assets, err => {
								if (err) {
									return callback(makeWebpackError(err, "Compilation.hooks.finishAssets"));
								}
			...
			// HotModuleReplacementPlugin 热更新相关，未使用热更新返回的是undefined
			consola.info('🔶 此处处理热更新监听的`shouldGenerateChunkAssets钩子`')
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
				if (!chunkGraph.getModuleHash(module)) {
					const moduleHash = createHash(hashFunction);
					module.updateHash(moduleHash, chunkGraph);
					const moduleHashDigest = /** @type {string} */ (moduleHash.digest(hashDigest));
					chunkGraph.setModuleHashes(
						module,
						moduleHashDigest,
						moduleHashDigest.substr(0, hashDigestLength)
	...
	emitAsset(file, source, assetInfo = {}) {
		if (this.assets[file]) {
			if (!isSourceEqual(this.assets[file], source)) {
				this.errors.push(new WebpackError(`Conflict: Multiple assets emit different content to the same filename ${file}`));
	...
	createChunkAssets(callback) {
		consola.info(`2⃣️ 4⃣️  执行\`createChunkAssets\`方法开始生成最终的静态资源`)
		const outputOptions = this.outputOptions;
		const cachedSourceMap = new WeakMap();
		const alreadyWrittenFiles = new Map(); /** @type {Map<string, {hash: string, source: Source, chunk: Chunk}>} */

		asyncLib.forEach(this.chunks, (chunk, callback) => {
			let manifest; /** @type {RenderManifestEntry[]} */
			try {
				consola.info('2⃣️ 5⃣️  触发`renderManifest`钩子')
				...
				consola.info('2⃣️ 7⃣️  chunk的`manifest`生成完毕')
			}
			catch (err) {
				this.errors.push(new ChunkRenderError(chunk, "", err));
				return callback();
			...
			asyncLib.forEach(manifest, (fileManifest, callback) => {
				...
				this.cache.get(cacheName, usedHash, (err, sourceFromCache) => {
					...
					const errorAndCallback = err => {
						const filename = file || (typeof filenameTemplate === "string" ? filenameTemplate : "");
					...
					try {
						...
						if (alreadyWritten !== undefined) {
							...
							// render the asset
							consola.info('2⃣️ 8⃣️  执行`manifest`的render方法生成最终`soucre`')
						...
						alreadyWrittenFiles.set(file, {hash: usedHash, source, chunk });
