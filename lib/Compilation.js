...
class Compilation {
	...
	_processModuleDependencies(module, callback) {
		...
		consola.error('下面开始解析Module所的依赖项目')
		asyncLib.forEach(sortedDependencies,
			(item, callback) => {
	...
	handleModuleCreation({ factory, dependencies, originModule, context }, callback) {
		...
		consola.info(`1⃣️ 2⃣️  进入\`handleModuleCreation\`方法，执行\`factorizeModule\`方法`)
		// factorize =》 因式分解
	...
	seal(callback) {
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			...
			consola.info(`开始触发optimizeTree之后的一系列钩子`)
			...
			consola.info(`处理热更新监听的shouldGenerateChunkAssets钩子`)
	...
	createChunkAssets(callback) {
		consola.info(`1⃣️  6⃣️  执行createChunkAssets方法开始生成最终的静态资源`)
		const outputOptions = this.outputOptions;
		...
		asyncLib.forEach(this.chunks, (chunk, callback) => {
			...
			try {
				consola.info('调用`renderManifest`钩子')
