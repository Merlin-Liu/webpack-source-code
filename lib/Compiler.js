...
class Compiler {
	 ...
	 */
	run(callback) {
		consola.info('5、执行Complier的run方法')
				...
				({ name: file, source, info }, callback) => {
					...
					const writeOut = err => {
						...
						const alreadyWritten = () => {
							...
							consola.info('打包后的文件内容未发生变化' + stack)
						...
						const doWrite = content /* buffer */ => {
							consola.info('开始执行写文件操作，生成的文件名：', targetPath);
							...
							if (content.length === stats.size) {
								...
								return this.outputFileSystem.readFile(
									...
									(err, existingContent) => {
										...
										// 文件已经存在，如果进入到这里，证明打包后的结果未曾发生过变化
										else {
											return alreadyWritten();
										}
									}
	 ...
	 */
	compile(callback) {
		consola.info('6、执行Complier的compile方法')
		...
		this.hooks.beforeCompile.callAsync(params, err => {
			...
			consola.info('7、创建Compilation实例')
			...
			consola.info('8、开始触发「make」生命周期钩子')
			this.hooks.make.callAsync(compilation, err => {
				consola.warn(`监听「make」生命周期「EntryPlugin」插件执行完成`)
				debugger
				...
				process.nextTick(() => {
					...
					compilation.finish(err => {
						...
						// seal =》 封装
						consola.success(`📦  依赖收集完成，开始执行seal方法`)
