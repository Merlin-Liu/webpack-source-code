 ...
 */
const getTaskForFile = (file, asset, options, compilation, cacheIdent, cacheETag) => {
	let source;
	/** @type {SourceMap} */
	let sourceMap;
	...
	return { file, asset, source, sourceMap, modules, cacheIdent, cacheETag };
};
...
class SourceMapDevToolPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap("SourceMapDevToolPlugin", compilation => {
			...
			compilation.hooks.finishAssets.tapAsync("SourceMapDevToolPlugin", (assets, callback) => {
				...
				for (const file of Object.keys(assets)) {
					matchObject(file) && files.push(file);
				}
				...
				asyncLib.each(files, (file, callback) => {
						...
						compilation.cache.get(cacheIdent, cacheETag, (err, assets) => {
							...
							if (assets) {
								for (const cachedFile in assets) {
									if (cachedFile === file) {
										...
										compilation.emitAsset(cachedFile, assets[cachedFile], { development: true });
									}
									/**
									...
									if (cachedFile !== file) {
										...
										chunk !== undefined && chunk.auxiliaryFiles.add(cachedFile);
									}
								}
							...
							const task = getTaskForFile(file, asset, options, compilation, cacheIdent, cacheETag);
									...
									if (!moduleToSourceNameMapping.get(module)) {
										moduleToSourceNameMapping.set(
											...
											ModuleFilenameHelpers.createFilename(
												module,
												{ moduleFilenameTemplate: moduleFilenameTemplate, namespace: namespace },
					...
					err => {
						...
						for (let idx = 0; idx < allModules.length; idx++) {
							...
							sourceName = ModuleFilenameHelpers.createFilename(
								module,
								{ moduleFilenameTemplate: fallbackModuleFilenameTemplate, namespace: namespace },
								{ requestShortener, chunkGraph }
							);
						...
						asyncLib.each(tasks, (task, callback) => {
							...
							options.noSources && (sourceMap.sourcesContent = undefined);
							...
							if (sourceMapFilename) {
								...
								const pathParams = {
									...
									contentHash: /** @type {string} */ (createHash("md4").update(sourceMapString).digest("hex"))
								};
								let sourceMapFile = compilation.getPath(sourceMapFilename, pathParams);
								const sourceMapUrl = options.publicPath ?
									options.publicPath + sourceMapFile
									: relative(outputFs, dirname(outputFs, `/${file}`), `/${sourceMapFile}`);
								/**
								...
								compilation.emitAsset(sourceMapFile, asset, { development: true });
								if (chunk !== undefined) {
