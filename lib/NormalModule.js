...
class NormalModule extends Module {
	...
	doBuild(options, compilation, resolver, fs, callback) {
		consola.info(`1⃣️  4⃣️：执行NormalModule的doBuild方法，调用loader对文件进行解析`)
		...
		const processResult = (err, result) => {
			...
			consola.info(`1⃣️  5⃣️：loader调用完成，开始生成NormalModule的_source`)
	...
	build(options, compilation, resolver, fs, callback) {
		consola.info(`执行NormalModule的build方法，该Module为${this.rawRequest}，Module的路径为${this.userRequest}`)
