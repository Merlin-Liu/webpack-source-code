...
const { CachedSource, OriginalSource, RawSource, SourceMapSource } = require("webpack-sources");
...
const { compareLocations, concatComparators, compareSelect, keepOriginalOrder } = require("./util/comparators");
...
class NormalModule extends Module {
	 ...
	 * @param {TODO} options.parser the parser used
	...
	doBuild(options, compilation, resolver, fs, callback) {
		consola.info(`1⃣️ 8⃣️  执行NormalModule的\`doBuild\`方法，调用loader对文件进行解析`)
		...
		const processResult = (err, result) => {
			if (err) {
				if (!(err instanceof Error)) err = new NonErrorEmittedError(err);
				const currentLoader = this.getCurrentLoader(loaderContext);
			...
			consola.info(`1⃣️ 9⃣️  loader调用完成，执行NormalModule的\`createSource\`方法生成\`this._source\``)
		...
		runLoaders(
			...
			(err, result) => {
				if (!result) {
					processResult(err || new Error("No result from loader-runner processing"), null);
				}
	...
	build(options, compilation, resolver, fs, callback) {
		consola.info(`1⃣️ 7⃣️  执行NormalModule的\`build\`方法，该Module为\`${this.rawRequest}\`，Module的路径为\`${this.userRequest}\``)
		...
		return this.doBuild(options, compilation, resolver, fs, err => {
			...
			const handleParseError = e => {
				const source = this._source.source(); /** @type {string} */
			...
			const handleParseResult = result => {
				this.dependencies.sort(
					concatComparators(compareSelect(a => a.loc, compareLocations), keepOriginalOrder(this.dependencies))
				);
			...
			let inTry = true;
			consola.info(`2⃣️ 0⃣️  执行NormalModule的\`doBuild\`的回调函数，开始解析Module`)
			try {
				const result = this.parser.parse(this._ast || this._source.source(), {current: this, module: this, compilation, options}, (err, result) => {
					inTry = false;
					if (err) {
				...
				if (result !== undefined) {
					handleParseResult(result); // parse is sync
				}
			}
			catch (e) {
				if (!inTry) throw e;
	...
	needBuild({ fileSystemInfo }, callback) {
		consola.info('1⃣️ 6⃣️  执行NormalModule的`needBuild`方法判断是否需要重新构建')
