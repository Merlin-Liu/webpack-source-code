...
class JavascriptParser {
	...
	blockPrewalkStatement(statement) {
		...
		switch (statement.type) {
			case "ImportDeclaration": //import声明
	...
	blockPrewalkImportDeclaration(statement) {
		...
		consola.info('🈶 解析source的时候识别到了一个模块导入语句，触发`import`钩子')
		this.hooks.import.call(statement, source);
		for (const specifier of statement.specifiers) {
			const name = specifier.local.name;
			switch (specifier.type) {
				case "ImportDefaultSpecifier": // 导入默认说明符
					if (!this.hooks.importSpecifier.call(statement, source, "default", name)) {s
	...
	parse(source, initialState) {
		consola.info(`2⃣️ 1⃣️  执行\`JavaScriptParser\`的\`parse\`方法解析Module的\`source\`生成AST语法树`)
		...
		if (this.hooks.program.call(ast, comments) === undefined) {
			...
			consola.info(`2⃣️ 2⃣️  遍历生成的AST语法树，识别导入模块，进行依赖模块收集`)
